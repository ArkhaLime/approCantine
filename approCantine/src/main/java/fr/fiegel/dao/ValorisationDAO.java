...
public class ValorisationDAO extends AbstractDAO<Valorisation> {
	...
	public Valorisation getValorisation() throws SQLException{
		String requete = "select "+COLONNE_VALEUR_TOT+", "+COLONNE_VALEUR_PERIME+" from (select sum(quantite*prix_achat) as "+COLONNE_VALEUR_TOT+" from produit) as t1, (select sum(quantite*prix_achat) as "+COLONNE_VALEUR_PERIME+" from produit where datediff(date_peremption,curdate())<=0) as t2";
		try {
